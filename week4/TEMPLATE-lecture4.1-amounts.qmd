...
jobs_clean |> 
  ...
  mutate(occupation = fct_reorder(.f = occupation, 
                                  .x = total_earnings)) |> 
  ...
  scale_y_continuous(labels = scales::label_currency(accuracy = 1, 
                                                     scale = 0.001, 
                                                     suffix = "k")) + 
  coord_flip() # or can just rev aesthethics
...
jobs_clean |> 
  ...
  geom_linerange(aes(ymin = 0,
                     ymax = total_earnings)) + # creates straight lines, must give it xmin,xmax,or ymin,ymax 
  coord_flip() +
  scale_y_continuous(labels = scales::label_currency(scale = 0.001, suffix = "k"),
                     limits = c(0,250000)) +
...
jobs_clean |>
  ...
  geom_linerange(aes(ymin = 0, ymax = earnings_by_group),
                 position = position_dodge(width = 0.75)) + # geom_linerange can start at non-zero, must set ymin=0 to prevent
  coord_flip()
```
...
ggplot(heatmap_order, aes(x = year, y = occupation, fill = total_earnings)) +
  ...
  coord_fixed() +
  scale_fill_viridis_b(labels = label_currency(scale = 0.001, suffix = "k")) +
  scale_y_discrete(label = label_wrap(30)) +
...
ggplot(subset_jobs) +
  ...
  geom_point(aes(x =total_earnings_male, y = occupation),
             color = "seagreen2", size = 2.5) +
  geom_point(aes(x =total_earnings_female, y = occupation),
             color = "indianred2", size = 2.5) +
  facet_wrap(~group_label, 
             nrow = 3,
             scales = "free_y") + # free_y ensures each facet only have applicable data
