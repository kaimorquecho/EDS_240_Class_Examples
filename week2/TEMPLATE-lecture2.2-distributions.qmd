...
see_NAs <- mko_clean %>%
  group_by(year) %>%
  naniar::miss_var_summary() %>% # gathers summary of missing data
...
mko_clean %>%
  mutate(month_name = factor(month_name, levels = month.name)) %>% # we want to make month_name into factor, month.name is vector included in R, we used it to give all the month names without typing them out
  ggplot(aes(x = Temp_bot, fill = month_name)) + # no y axis because it will be the count for months 
  geom_histogram(position = "identity",
                 alpha = 0.5) # to unstack the bars
...
mko_clean %>%
  ...
  geom_histogram(position = "identity",
                 alpha = 0.5,
                 color = "black",
                 binwidth = 1) + # changes size of bin to include only 1 degree, 
...
mko_clean |> 
  filter(month_name %in% c("April", "June", "October")) |> 
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_density(alpha = 0.5, adjust = 0.5 ) + # adjust makes density plot less smooth
  scale_fill_manual(values = c("#2C5374", "#ADD8E6", "#8B3A3A"))
```
...
ggplot(mko_clean, aes(x = Temp_bot, y = month_name)) +
  ggridges::geom_density_ridges(rel_min_height = 0.01,
                                scale = 3) # rel_min_height sets threshhold for our x axis limits
# scale adjusts the overlap of each ridge
...
ggplot(mko_clean, aes(x = Temp_bot, y = month_name, fill = after_stat(x))) +
  ...
  scale_y_discrete(limits = rev(month.name)) # gets month names in chronological order, rev = reverses order 
...
ggplot(mko_clean, aes(x = month_name,
                      y = Temp_bot)) +
  geom_boxplot() +
  scale_x_discrete(limits = month.name) # organize x-axis by month order reversed
...
ggplot(mko_clean, aes(x = month_name, 
                      ...
                      fill = month_name)) +
  geom_boxplot() +
  scale_x_discrete(limits = rev(month.name)) + # discrete variable and x axis 
...
ggplot(penguins, aes(x = species, y = body_mass_g)) +
 ...
 # if adding data points jittered on top, ensure to remove outliers to prevent duplicate data points
```
